#!/usr/bin/env python3
"""Create lightweight PDF and PPTX exports without third-party packages."""
...
def update_manifest() -> None:
    path = ROOT / "capstone-manifest.json"
    if not path.exists():
        return
    manifest = json.loads(path.read_text(encoding="utf-8"))
    outputs = manifest.setdefault("outputs", {})
    outputs["capstone_deck_pptx"] = str(ROOT / "capstone-deck.pptx")
    outputs["print_packet_pdf"] = str(ROOT / "print-packet.pdf")
...
def main() -> None:
    make_pdf(ROOT / "print-packet.md", ROOT / "print-packet.pdf")
    make_pptx(ROOT / "capstone-deck.md", ROOT / "capstone-deck.pptx")
    update_manifest()
